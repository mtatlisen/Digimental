        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {NAV.map(item => (
                            <a key={item.href} href={item.href} className="block text-gray-500 hover:text-emerald-400 text-sm transition-colors">
                                {item.label}
                            </a>
                        ))}
                    </div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {BRANDS.map(b => (
                            <a key={b.name} href={b.url} target="_blank" className="block text-gray-500 hover:text-emerald-400 text-sm transition-colors">
                                {b.flag} {b.name}
                            </a>
                        ))}
                    </div>
            ...
            <div className="border-t border-white/5 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-6">
                    {BRANDS.map(b => (
                        <a key={b.name} href={b.url} target="_blank" className="text-2xl hover:scale-110 transition-transform">
                            {b.flag}
                        </a>
                    ))}
                </div>
